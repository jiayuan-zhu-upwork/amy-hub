...
WITH base AS (
    ...
    SELECT
        uvs.first_upwork_location
        , uvs.marketing_channel
...
final AS (
    SELECT
        case when first_upwork_location in ('homepage','pricing', 'i_how-it-works') then first_upwork_location 
            when first_upwork_location = 'hire' then
            case when marketing_channel = 'SEO' then 'hire_seo'
                when marketing_channel = 'SEM' then 'hire_sem'
                else 'hire_other' end
            else 'other'
        end as first_upwork_location
        , week_
, visitors
, visitors / NULLIF(LAG(visitors, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS visitors_wow_pct
, visitors / NULLIF(LAG(visitors, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS visitors_vs_last_mo_pct

        , page_ctr
        , page_ctr / NULLIF(LAG(page_ctr, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1   AS ctr_wow_pct
        , page_ctr / NULLIF(LAG(page_ctr, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1   AS ctr_vs_last_mo_pct

        , visit_to_cl_reg_conversion_1d
        , visit_to_cl_reg_conversion_1d / NULLIF(LAG(visit_to_cl_reg_conversion_1d, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS cl_reg_wow_pct
        , visit_to_cl_reg_conversion_1d / NULLIF(LAG(visit_to_cl_reg_conversion_1d, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS cl_reg_vs_last_mo_pct

        , visit_to_smb_signup_conversion_1d
        , visit_to_smb_signup_conversion_1d / NULLIF(LAG(visit_to_smb_signup_conversion_1d, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS smb_reg_wow_pct
        , visit_to_smb_signup_conversion_1d / NULLIF(LAG(visit_to_smb_signup_conversion_1d, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS smb_reg_vs_last_mo_pct

        , visit_to_bp_signup_conversion_1d
        , visit_to_bp_signup_conversion_1d / NULLIF(LAG(visit_to_bp_signup_conversion_1d, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1   AS bp_reg_wow_pct
        , visit_to_bp_signup_conversion_1d / NULLIF(LAG(visit_to_bp_signup_conversion_1d, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1   AS bp_reg_vs_last_mo_pct

        , median_duration_seconds
        , median_duration_seconds / NULLIF(LAG(median_duration_seconds, 1) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS dur_wow_pct
        , median_duration_seconds / NULLIF(LAG(median_duration_seconds, 4) OVER (PARTITION BY first_upwork_location ORDER BY week_ ASC), 0) - 1  AS dur_vs_last_mo_pct
...
unpivoted AS (
SELECT 1 AS metric_order, 'Visits' AS metric, first_upwork_location, visitors AS wk_val, visitors_wow_pct AS wow, visitors_vs_last_mo_pct AS vs_mo FROM latest
    UNION ALL
    SELECT 2,                 'CTR',              first_upwork_location, page_ctr,                          ctr_wow_pct,               ctr_vs_last_mo_pct                           FROM latest
    UNION ALL
    SELECT 3,                 'CL Reg %',         first_upwork_location, visit_to_cl_reg_conversion_1d,    cl_reg_wow_pct,            cl_reg_vs_last_mo_pct                        FROM latest
    UNION ALL
    SELECT 4,                 'SMB Reg %',        first_upwork_location, visit_to_smb_signup_conversion_1d, smb_reg_wow_pct,          smb_reg_vs_last_mo_pct                       FROM latest
    UNION ALL
    SELECT 5,                 'B+ Reg %',         first_upwork_location, visit_to_bp_signup_conversion_1d,  bp_reg_wow_pct,           bp_reg_vs_last_mo_pct                        FROM latest
    UNION ALL
    SELECT 6,                 'Median session (s)', first_upwork_location, median_duration_seconds,         dur_wow_pct,               dur_vs_last_mo_pct                           FROM latest
)
...
SELECT
    ...
    -- Homepage
    , MAX(CASE WHEN first_upwork_location = 'homepage'          THEN wk_val END)  AS hp_wk
    , MAX(CASE WHEN first_upwork_location = 'homepage'          THEN wow    END)  AS hp_wow
    , MAX(CASE WHEN first_upwork_location = 'homepage'          THEN vs_mo  END)  AS hp_vs_mo

    -- Hire SEO
    , MAX(CASE WHEN first_upwork_location = 'hire_seo'              THEN wk_val END)  AS hire_wk
    , MAX(CASE WHEN first_upwork_location = 'hire_seo'              THEN wow    END)  AS hire_wow
    , MAX(CASE WHEN first_upwork_location = 'hire_seo'              THEN vs_mo  END)  AS hire_vs_mo

    -- Hire SEM
    , MAX(CASE WHEN first_upwork_location = 'hire_sem' THEN wk_val END) AS hls_wk
    , MAX(CASE WHEN first_upwork_location = 'hire_sem' THEN wow    END) AS hls_wow
    , MAX(CASE WHEN first_upwork_location = 'hire_sem' THEN vs_mo  END) AS hls_vs_mo

    -- Pricing
    , MAX(CASE WHEN first_upwork_location = 'pricing'           THEN wk_val END)  AS pricing_wk
    , MAX(CASE WHEN first_upwork_location = 'pricing'           THEN wow    END)  AS pricing_wow
    , MAX(CASE WHEN first_upwork_location = 'pricing'           THEN vs_mo  END)  AS pricing_vs_mo

    -- How It Works
    , MAX(CASE WHEN first_upwork_location = 'i_how-it-works'   THEN wk_val END)   AS hiw_wk
    , MAX(CASE WHEN first_upwork_location = 'i_how-it-works'   THEN wow    END)   AS hiw_wow
    , MAX(CASE WHEN first_upwork_location = 'i_how-it-works'   THEN vs_mo  END)   AS hiw_vs_mo
